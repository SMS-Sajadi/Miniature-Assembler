...
char* inst[] = {"add","sub","slt","or","nand","addi","slti","ori","lui","lw","sw","beq","jalr","j","halt"};
...
void fillsymTab(struct symbolTable* symtab, FILE* inputfile)
{
    int nofline = 0, i = 0;
    size_t line_size;
    char* line = (char*)malloc(72);
...
//change to bool
int check_symbol_table(struct symbolTable* table, int size)
{
    for(int i = 0; i < size; i++)
    {
        ...
        {
            if(i == j) continue;
            if(strcmp(table[i].symbol, table[j].symbol) == 0) return 1;
        }
    }
    return 0;
}
...
int main(int argc, char** argv) {
    ...
    table = (struct symbolTable*)malloc(symtab_len * sizeof(struct symbolTable));
